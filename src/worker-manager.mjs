...
export class WorkerManager {
  ...
  async start({ task, cwd = process.cwd(), supervisorThreadId = '' } = {}) {
    ...
    await this.taskStore.write(record);    const gatewayToken = await this.store.gatewayToken();
  ...
  async #leaseExpired(taskId) {
    const active = this.active.get(taskId);
    if (!active || terminal(active.record)) return;    const deadline = Date.parse(active.record.supervision.leaseDeadlineAt || '') || 0;
  ...
  #serverRequest(record, request) {
    ...
    if (record.pendingInteractions.length > MAX_PENDING_INTERACTIONS) {
      record.pendingInteractions.splice(0, record.pendingInteractions.length - MAX_PENDING_INTERACTIONS);
    }
