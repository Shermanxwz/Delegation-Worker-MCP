...
export const CODEX_GATEWAY_TOKEN_ENV = 'DWMCP_GATEWAY_TOKEN';
export const CODEX_GATEWAY_BASE_URL_ENV = 'DWMCP_GATEWAY_BASE_URL';
...
export class CodexConfigManager {
  ...
  runtimeEnv(token, extra = {}) {
    ...
    if (!value) throw new Error('gateway token is required for process-scoped Codex runtime configuration');
    return {
      ...this.env,
      ...extra,
      [CODEX_GATEWAY_TOKEN_ENV]: value,
      [CODEX_GATEWAY_BASE_URL_ENV]: this.gatewayBaseUrl
    };
  }

  async status() {
    const text = await this.read();
    const legacyInstalled = text.includes(START) || text.includes(`[model_providers.${CODEX_PROVIDER_ID}]`);
    return {
      installed: true,
      processScoped: true,
      legacyInstalled,
      ...
      selectors: inspectTopLevelSelectors(text)
    };
  }

  async install() {
    const cleaned = await this.#removeLegacyManagedProvider();
    return { ...cleaned, migrated: cleaned.removedLegacyProvider };
  }

  async uninstall() {
    return this.#removeLegacyManagedProvider();
  }

  async #removeLegacyManagedProvider() {
    await fs.mkdir(this.home, { recursive: true, mode: 0o700 });
    return withFileLock(this.file, async () => {
      const before = await this.read();
      const selectors = inspectTopLevelSelectors(before);
      const next = removeManaged(before);
      if (!sameSelectors(selectors, inspectTopLevelSelectors(next))) {
        throw new Error('refusing to modify official top-level Codex model/provider selectors');
      }
      const changed = next.trimEnd() !== before.trimEnd();
      if (changed) await atomicWrite(this.file, next ? `${next}\n` : '', { mode: 0o600 });
      const status = await this.status();
      return { removed: true, removedLegacyProvider: changed, ...status };
    });
  }
}
